...
`));
  ...
  t.test('make sure prettier prevents warnigns on things like quotes', t => {
    t.plan(3);
    const resultDoubleQuote = lint('"foobar"');
    const resultSingleQuote = lint("'foobar'");

    t.deepEquals(resultDoubleQuote.messages, [], 'no messages in results');
    t.deepEquals(resultSingleQuote.messages, [], 'no messages in results');
  });
